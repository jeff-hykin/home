...
const { run, Timeout, Env, Cwd, Stdin, Stdout, Stderr, Out, Overwrite, AppendTo, zipInto, mergeInto, returnAsString, } = await import(`https://deno.land/x/quickr@0.6.24/main/run.js`)
import { FileSystem } from "https://deno.land/x/quickr@0.6.43/main/file_system.js"
// const { FileSystem } = await import(`./file_system.js`)
const { Console, cyan, white } = await import(`https://deno.land/x/quickr@0.6.24/main/console.js`)
...
async function findAllKeyPairs(sshFolder) {
    ...
    const keyPairs = pubKeyName.filter(each=>noPubNames.includes(each)).map(each=>[ `${sshFolder}/${each}`, `${sshFolder}/${each}.pub` ])
    return keyPairs
}
