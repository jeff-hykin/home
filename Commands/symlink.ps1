...
import { parseArgs, flag, required, initialValue } from "https://deno.land/x/good@1.6.1.2/flattened/parse_args.js"
// import { parseArgs, flag, required, initialValue } from "/Users/jeffhykin/repos/good-js/source/flattened/parse_args.js"
import { FileSystem, glob } from "https://deno.land/x/quickr@0.6.66/main/file_system.js"

var output
const { existing: existingPath, new: newPath, absolute: isAbsolute, allowNonExistingTarget, hardLink, overwrite } = output = parseArgs({
    rawArgs: Deno.args,
    fields: [
        [["--existing", ], required, ],
        [["--new"], required, ],
        [["--absolute", ], flag, initialValue(false), JSON.parse ],
        [["--allowNonExistingTarget", ], flag, initialValue(false), JSON.parse ],
        [["--hardLink", ], flag, initialValue(false), JSON.parse ],
        [["--overwrite", ], flag, initialValue(false), JSON.parse ],
    ],
    allowNameRepeats: false,
    valueTransformer: (arg)=>arg, // e.g. keep as-is
}).simplifiedNames

if (hardLink) {
    await FileSystem.hardLink({ existingItem: existingPath, newItem: newPath, force: true, allowNonExistingTarget, overwrite  })
} else if (isAbsolute) {
    await FileSystem.absoluteLink({ existingItem: existingPath, newItem: newPath, force: true, allowNonExistingTarget, overwrite  })
} else {
    await FileSystem.relativeLink({ existingItem: existingPath, newItem: newPath, force: true, allowNonExistingTarget, overwrite  })
}
