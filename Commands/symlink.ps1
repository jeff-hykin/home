...
import { parseArgs, flag, required, initialValue } from "https://deno.land/x/good@1.6.1.1/flattened/parse_args.js"
// import { parseArgs, flag, required, initialValue } from "/Users/jeffhykin/repos/good-js/source/flattened/parse_args.js"
import { FileSystem, glob } from "https://deno.land/x/quickr@0.6.65/main/file_system.js"

var output
const { existing: existingPath, new: newPath, absolute: isAbsolute, allowNonExistingTarget, hardLink } = output = parseArgs({
    rawArgs: Deno.args,
    fields: [
        [["--existing", ], required, ],
        [["--new"], required, ],
...
if (isAbsolute) {
    await FileSystem.absoluteLink({ existingItem: existingPath, newItem: newPath, force: true, allowNonExistingTarget, hardLink, })
} else {
    await FileSystem.relativeLink({ existingItem: existingPath, newItem: newPath, force: true, allowNonExistingTarget, hardLink, })
}
